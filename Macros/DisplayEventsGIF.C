...
for(int l=0;l<e->get_Nhits();l++)
     {
             ...
             double cz = e->get_hits().at(l)->get_cz();
                 if(Lindex==0) {EL0 = 1000*(e->get_hits().at(l)->get_eMC());}
        } //end loop on hits  
...
//   cout << "Extraction of bending plane fit parameters done" <<endl;
  ...
  //  else fNB->DrawF1(fNB->GetX(40),fNB->GetX(-30),"same");
   ...
   // ThetaNB->Draw();
	...
	TPaveText*PNL=new TPaveText(16,-10,30,-20);
	PNL->AddText(Form("%d layers with hits",NL));    
        PNL->AddText(Form("E at L0 = %2.2f MeV", EL0));        
