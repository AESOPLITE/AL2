...
//Temporary variable for internal triggers
 int*Titmp=new int[nl];
 for (int i=0;i<nl;i++)Titmp[i]=0;
 //Temporary variable for internal triggers with max 3 strips
 int*Tictmp=new int[nl];
 for (int i=0;i<nl;i++)Tictmp[i]=0;
      ...
      if(i==0)//if first event
       {
        ...
        // e->set_T2(false);
	      for(int ij=0;ij<nl;ij++)Titmp[ij]=0;
       }//if(i==0)
      else
       {
        //Fill internal trigger information of the previous event
        int t=0;
        for (int ij=0;ij<nl;ij++) t+=Titmp[ij]*(int)TMath::Power(2,ij);
        e->set_Ti(t);
	      int ttc=0;
        for (int ij=0;ij<nl;ij++) ttc+=Tictmp[ij]*(int)TMath::Power(2,ij);
        e->set_Tic(ttc);
 ...
 int tt=0;
 for (int ij=0;ij<nl;ij++) tt+=Titmp[ij]*(int)TMath::Power(2,ij);
 e->set_Ti(tt);
