  ...
  //cout << endl;
	...
	for (int lyr = 0; lyr < tD->nLayers; lyr++) {
		...
		if (tD->orientation[lyr] == 'n') {
			...
			double incr = pow((xInterp - xMeas) / tD->sigma[lyr].at(hits[lyr]), 2);
			result += incr;
			delete[] rInterp;
			...
			double incr = pow((yInterp - yMeas) / tD->sigma[lyr].at(hits[lyr]), 2);
			result += incr;
			delete[] rInterp;
...
double RKfitter::chi2m(double a[]) {
	...
	for (int i = 0; i < tD->nLayers; i++) {
		...
		Cx[i][i] = pow(tD->sigma[i].at(hits[i]), 2);  // Including here the measurement uncertainties in the diagonal elements
	...
	*/
	...
	if (verbose) {
		cout << "RKfitter::chi2: with mult scat=" << multScat << "  a=";
		for (int i = 0; i < 5; i++) cout << a[i] << " ";
		cout << "  chi^2=" << result << endl;
	}
	return result;
...
void RKfitter::hessian(double hin[5]) {
	double fxy = chi2(a);
  //cout << "RKfitter::hessian, a= " << a[0] << " " << a[1] << " "  << a[2] << " "  << a[3] << " "  << a[4] << " chi2=" << fxy << endl;
	double h[5];
	for (int i = 0; i < 5; i++) {
		h[i] = hin[i] / 2.;
		double delta;
    int ntry=0;
      ...
      ntry++;
		} while (abs(delta) < 0.00001 && ntry < 10);
...
int RKfitter::fitIt(bool genStartGuess, double guess[5], std::vector<int> hitSelection) {
  ...
  //if (tD->eventNumber == 46523) verbose = true;
	...
	switch(algorithm) {
	case 0:
    //cout << "RKfitter::fitIt: calling nelmin with maxCalls=" << maxCalls << endl;
...
/*		try {
  ...
  if (ynewlo < 100000.) {
      ...
      //cout << "RKfitter::fitit, inverting the hessian matrix now" << endl;
      ...
      cout << "RKfitter::fitit, skipping Hessian calculation due to large chi^2" << endl;
      ifault = ifault + 10;
  }
...
*/
	...
	delete[] temp;
  if (ifault != 0) cout << "RKfitter: exiting fitit with code " << ifault << endl;
	return ifault;
}
...
void RKfitter::print(string s) {
	cout << endl;
	cout << "RKfitter " << s << " fit results for hits ";
