...
int ALPatternRecognition::FindPattern(ALEvent *re, int DataType, float* zL,float* OffsetLL,float* OffsetRL,float* TrigThresh)
{
 ...
 TObjArray *xbend5 = new TObjArray();
 vector<int> ibend1, ibend2, ibend3, ibend4, ibend5;//keep track of index of hit
 vector<int> itop, imid, ibottom;	//keep track of index of hit
 vector<int> nstrip1, nstrip2, nstrip3,nstrip4,nstrip5;   //keep track of number of strips that make up a hit
 ...
 int* NLPRtmp= new int[nbl];
 for(int i=0;i<nbl;i++)NLPRtmp[i]=0;
       ...
       switch(L)
            {
             ...
             case 4: if (abs(re->get_hits().at(j)->get_y())<10)   {xbend4->Add(X);ibend4.push_back(kk);nstrip4.push_back(nstrip);  NL4=true;NLPRtmp[L]++;}break;
             case 6: if (abs(re->get_hits().at(j)->get_y())<10)   {xbend5->Add(X);ibend5.push_back(kk);nstrip5.push_back(nstrip);  NL6=true;NLPRtmp[L]++;}break;
            }
      }//else
      //cout << "Hit " << j << "  , L=" << L << ", fstripID " << fstripID << "  nstrip =  " << nstrip << "    x=" << xx << "  y=" << yy << "  z=" << zz << endl;   
 ...
 re->set_TiPR(TiPRtmp);
 //cout << " TiPR = " << (int)TiPRtmp << endl;
 //GraphAllPoints(re);

 //TEST ALL CONFIGURATIONS BENDING PLANE
 int npointsB, layer1, layer2, layer3, layer4,layer5, ncomb;
 layer1=layer2=layer3=layer4=layer5=1;
 npointsB = xbend->GetEntries();
 int NLB = (int)NL1 + (int)NL2 + (int)NL3 +(int)NL4 +(int)NL6;
 ...
 if((xbend5->GetEntries())!=0) layer5 = xbend5->GetEntries() ;
 ncomb = layer1*layer2*layer3*layer4*layer5;
       ...
       for(int o=0; o<layer3; o++)
         {
          for(int p=0;p<layer4;p++)
            {
             for(int q=0;q<layer5;q++)
               {
	    
                indicesB[ncombination] = new TArrayI(5);
 	          ...
 	          gBending[ncombination]->SetPoint(gBending[ncombination]->GetN(), xx1->Y(), xx1->Z());//first layer
       	          //cout << "y1 = " << xx1->Y() << " z1 = " << xx1->Z() << endl;
       	          gBendingInverted[ncombination]->SetPoint(gBendingInverted[ncombination]->GetN(), xx1->Z(), xx1->Y());//first layer
                  if(nstrip1.at(m)==4) sigma1= 3*sigma_reso;
                  else if(nstrip1.at(m)==3) sigma1= 2*sigma_reso;
                ...
                if(xbend2->GetEntries()!=0)
         	 {
         	  ...
         	  //cout << "y2 = " << xx2->Y() << " z2 = " << xx2->Z() << endl;
      	 	  gBendingInverted[ncombination]->SetPoint(gBendingInverted[ncombination]->GetN(), xx2->Z(), xx2->Y());//second layer
                  if(nstrip2.at(n)==4) sigma2= 3*sigma_reso;
                  else if(nstrip2.at(n)==3) sigma2= 2*sigma_reso;
         	  ...
         	  //cout << "y3 = " << xx3->Y() << " z3 = " << xx3->Z() << endl;
                  if(nstrip3.at(o)==4) sigma3= 3*sigma_reso;
                  else if(nstrip3.at(o)==3) sigma3= 2*sigma_reso;
         	  ...
         	  //cout << "y4 = " << xx4->Y() << " z4 = " << xx4->Z() << endl;
                  if(nstrip4.at(p)==4) sigma4= 3*sigma_reso;
                  else if(nstrip4.at(p)==3) sigma4= 2*sigma_reso;
         	  ...
         	  gBendingInverted[ncombination]->SetPointError(gBendingInverted[ncombination]->GetN()-1,0.,sigma4);
                ...
                if(xbend5->GetEntries()!=0)
                 {
                  // cout << " xbend5 " << endl;
                  TVector3 *xx5 =(TVector3*)xbend5->At(q);
                  gBending[ncombination]->SetPoint(gBending[ncombination]->GetN(), xx5->Y(), xx5->Z());//fifth layer
                  gBendingInverted[ncombination]->SetPoint(gBendingInverted[ncombination]->GetN(), xx5->Z(), xx5->Y());//fifth layer
                  //cout << "y5 = " << xx5->Y() << " z5 = " << xx5->Z() << endl;
                  if(nstrip5.at(q)==4) sigma5= 3*sigma_reso;
                  else if(nstrip5.at(q)==3) sigma5= 2*sigma_reso;
                  else if(nstrip5.at(q)==2) sigma5= sigma_reso/(TMath::Sqrt(2.));
                  else sigma5=sigma_reso;
      	          // cout << "nstrip5 = " << nstrip5.at(q) << " sigma5 = " << sigma5 << endl;
                  gBending[ncombination]->SetPointError(gBending[ncombination]->GetN()-1,sigma5,0.);
                  gBendingInverted[ncombination]->SetPointError(gBendingInverted[ncombination]->GetN()-1,0.,sigma5);
      	          nhitB[ncombination]++;
                 } 
         	 ...
         	 {
          	  int k5 = ibend5.at(q);
         	  indicesB[ncombination]->AddAt(k5,point);
         	  //cout << "PR, bending plane, point added at index " << k5 << endl;
         	  point++;
         	 }
                if(xbend4->GetEntries()!=0)
         	 {
 ...
 int NLNB = (int)NL0 + (int)NL5 + (int)NL7;
 ...
 for(int m=0; m<ntop; m++)
   {
    for(int n=0; n<nbottom; n++)
       ...
       for(int o=0; o<nmid; o++)
 	 {
 	  ...
 	  if(xtoplayer->GetEntries()!=0)
 	   {
 	    ...
 	    if(nstriptop.at(m)==4) sigmatop= 3*sigma_reso;
            else if(nstriptop.at(m)==3) sigmatop= 2*sigma_reso;
 	  ...
 	  if(xbottomlayer->GetEntries()!=0)
 	   {
 	    ...
 	    if(nstripbottom.at(n)==4) sigmabottom= 3*sigma_reso;
 	  ...
 	  if(xmid->GetEntries()!=0)
 	   {
 	    ...
 	    if(nstripmid.at(o)==4) sigmamid= 3*sigma_reso;
            else if(nstripmid.at(o)==3) sigmamid= 2*sigma_reso;
 	  ...
 	  //FIT
 	  //cout << "non-bending plane fit" << endl;
 	  gNonBending[NConfig]->Fit(line[NConfig],"QSN");
 	  //cout << "end non-bending plane fit" << endl;
 ...
 float limo=zL[6];//z position of 7th layer
